...
public class ExpectedPoints {
    ...
    /** Макисмальное количество времени, которое игрок мог провести на поле */
    private double totalTimePlayedInTournament = 1350;
    ...
    public void count() throws IOException {
        ...
        countTeamXg(parsedPlayers, fanTeamPlayers);
        readBookmakersData();
        readOpponentsMap();
     ...
     * @param fanTeamPlayers - объект "Игрок", полученный от фэнтези-провайдера.
     */
    private void countTeamXg(List<ParsedPlayer> parsedPlayers, List<FanTeamPlayer> fanTeamPlayers) {
        for (FanTeamPlayer fanTeamPlayer : fanTeamPlayers) {
            for (ParsedPlayer parsedPlayer : parsedPlayers) {
                if (isSamePlayer(fanTeamPlayer, parsedPlayer)) {
                    if (teamXgMap.containsKey(fanTeamPlayer.getTeam())) {
                        teamXgMap.put(
                                fanTeamPlayer.getTeam(),
                                teamXgMap.get(fanTeamPlayer.getTeam()) + parsedPlayer.getTotalXg()
                        );
                    } else {
                        teamXgMap.put(fanTeamPlayer.getTeam(), parsedPlayer.getTotalXg());
                    }
                }
            }
        }
    ...
    public double countPlayerPoints(FanTeamPlayer fanTeamPlayer, ParsedPlayer parsedPlayer) throws IOException {
        ...
        double timeShare = parsedPlayer.getMinutesPlayed() / totalTimePlayedInTournament;
    ...
    /**
     * Оценка очков за действия, награда за который зависит от позиции.
